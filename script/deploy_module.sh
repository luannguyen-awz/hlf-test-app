...
if helm list -n ${APP_NAMESPACE} | grep -q "^${APPLICATION_PREFIX}"; then
    echo "Release ${APPLICATION_PREFIX} already exists. Checking for changes..."

    # Get current revision
    CURRENT_REVISION=$(helm list -n ${APP_NAMESPACE} -o json | jq -r '.[] | select(.name=="'${APPLICATION_PREFIX}'") | .revision')

    helm upgrade ${APPLICATION_PREFIX} ./charts/${APPLICATION_PREFIX} \
    ...
    # Get new revision
    NEW_REVISION=$(helm list -n ${APP_NAMESPACE} -o json | jq -r '.[] | select(.name=="'${APPLICATION_PREFIX}'") | .revision')
    ...
    helm install ${APPLICATION_PREFIX} ./charts/${APPLICATION_PREFIX} \
        --namespace ${APP_NAMESPACE} \
        --set image.repository=${ECR_REPOSITORY} \
        --set image.tag=${IMAGE_TAG} \
        --set environment=${ENVIRONMENT} \
